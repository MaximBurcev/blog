...
class DiagramTranslatorService
{
    ...
    /**
     * Доля слов с кириллицей, начиная с которой текст считается уже
     * переведённым.
     *
     * Порог низкий намеренно: цены ошибок несимметричны. Не перевести — значит
     * оставить английскую обложку, перевести повторно — значит стереть русский
     * текст и нарисовать поверх мусор, причём файл переписывается на месте.
     * Замеры: техзаголовок «Разбор N+1 в Eloquent: Laravel Debugbar,
     * Telescope, Clockwork, Xdebug» — 0.22, английский с одной гомоглифной
     * ошибкой OCR — около 0.14.
     */
    private const CYRILLIC_SHARE = 0.2;

    /**
     * Короче этого текст на язык не проверяем.
     *
     * `eng+rus` изредка отдаёт кириллический глиф вместо похожего латинского, и
     * на метке в три буквы («CPU» → «СРU») одна подмена даёт долю выше любого
     * порога. Для диаграмм это сплошь короткие подписи, их лучше перевести.
     */
    private const MIN_LETTERS_TO_JUDGE = 8;
    ...
    public function translate(string $imagePath, bool $headingOnly = false): bool
    {
        try {
            ...
            // Решение о языке принимается один раз на всю картинку, а не по
            // параграфам. Русская обложка с английской плашкой в отдельном
            // блоке (psm 3 разносит их по разным block_num) иначе потеряла бы
            // эту плашку: её параграф не выглядит русским, ушёл бы в перевод и
            // перерисовался поверх нормальной картинки.
            if ($this->looksAlreadyTranslated(implode(' ', array_column($lines, 'text')))) {
                Log::info('DiagramTranslator: текст уже на русском, картинка пропущена', ['path' => $imagePath]);
    ...
    private function runOcr(string $imagePath): ?string
    {
        ...
        // eng+rus, а не один eng: часть обложек в архиве уже на русском, и
        // английская модель читала кириллицу как похожую латиницу — «Топ-16
        // обязательных ресурсов» превращалось в «Ton-16 pecypcosB», этот мусор
        // честно переводился и рисовался поверх нормального текста. С двумя
        // моделями русский распознаётся как русский (и отсеивается проверкой
        // ниже), а на английских картинках результат не меняется вовсе:
        // замерено на обложке dev.to — те же 19 слов и та же средняя
        // уверенность 88.
        $result = Process::timeout(self::OCR_TIMEOUT)
            ->run([$binary, $imagePath, 'stdout', '--psm', '3', '-l', 'eng+rus', 'tsv']);
        ...
        if ($result->failed()) {
            Log::warning('DiagramTranslator: OCR не отработал — текст на картинках не переводится', [
                ...
                'path' => getenv('PATH'),
                'hint' => match (true) {
                    $result->exitCode() === 127 => 'бинарь не найден: ./vendor/bin/envoy run ocr-install',
                    // Второй режим отказа появился вместе с `-l eng+rus`:
                    // пакет языка отсутствует, а код возврата обычная единица.
                    str_contains($result->errorOutput(), 'traineddata') => 'нет языкового пакета: ./vendor/bin/envoy run ocr-install',
                    default => null,
                },
            ]);
    ...
    /**
     * Текст на картинке уже по-русски — переводить нечего.
     *
     * Проверка появилась после массового прогона по архиву: часть обложек была
     * русской изначально, английская модель прочитала кириллицу как латиницу
     * («Топ-16 обязательных ресурсов» → «Ton-16 pecypcosB»), перевод этого
     * мусора нарисовался поверх нормального текста, и картинка превратилась в
     * кашу из наложенных строк. Файл при этом перезаписывается, то есть без
     * бэкапа потеря была бы безвозвратной.
     *
     * Порог, а не «есть хоть одна кириллическая буква»: на английской картинке
     * OCR изредка отдаёт одиночный кириллический символ вместо похожего
     * латинского, и из-за него переводить перестала бы вся обложка.
     */
    private function looksAlreadyTranslated(string $text): bool
    {
        // `< 1`, а не `=== 0`: при битом UTF-8 (а OCR его выдаёт) preg_match_all
        // возвращает false, которое строгому сравнению с нулём не равно — и
        // управление проваливалось бы в деление на false.
        if (preg_match_all('/\p{L}/u', $text) < self::MIN_LETTERS_TO_JUDGE) {
            return false;
        }

        // Считаем слова, а не буквы. Латинский термин весит 7-10 букв, русское
        // служебное слово — 2-3, поэтому по буквам «Разбор N+1 в Eloquent:
        // Laravel Debugbar, Telescope, Clockwork» даёт долю 0.13 и русская
        // обложка уходила бы в перевод. По словам это 2 из 9 — выше порога.
        ...
        $cyrillicWords = count(array_filter(
            $words,
            static fn (string $word): bool => preg_match('/\p{Cyrillic}/u', $word) === 1,
        ));

        return $cyrillicWords / count($words) >= self::CYRILLIC_SHARE;
    }
