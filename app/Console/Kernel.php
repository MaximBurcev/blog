...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule): void
    {
        ...
        //
        // Вернуть в расписание можно после того, как: (1) StorePostJob
        // перестанет переводить заголовок ДО успешного извлечения контента
        // (перевод на :916, проверка на пустой контент только на :198); (2) у
        // повторов заглушек появится потолок.
        //
        // Оговорка к (1): лишним запросом оплачивается не всякая заглушка, а
        // только та, что скачалась и отдала заголовок, но не дала контента по
        // селектору. Сбой скачивания, антибот и отсутствие <h1>/<title> до
        // перевода не доходят и стоят ноль — поэтому вызовов в llm_calls
        // меньше, чем заглушек в posts.
        //
        // Сама команда никуда не делась: `php artisan news:import [url]`.
        // Без аргумента она обходит все заведённые релизы, то есть ручной
        // запуск воспроизведёт ровно тот же холостой ход — до починки её лучше
        // звать с конкретным адресом дайджеста.
