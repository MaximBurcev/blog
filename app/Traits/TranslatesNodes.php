...
trait TranslatesNodes
{
    ...
    private function translateBlock(DOMElement $element): void
    {
        ...
        // Google иногда искажает служебные токены при переводе на русский:
        // вставляет пробелы (#{0} → # {0}) или подменяет "$" на "№" — особенно
        // часто именно в ПОСЛЕДНЕМ токене строки (воспроизведено стабильно:
        // "...how? ${3}" → "...как? №{3}"), видимо путает с русским «№n».
        // Нумерация #{i}/№{i} и ${i} совпадает (оба присваиваются
        // последовательно), поэтому донормализуем сами
     ...
     *
     * pre/script/style — всегда целиком код, маскируются без разбора.
     * code — тоже код по умолчанию, НО некоторые авторы оформляют им
     * риторические фразы для акцента (напр. <code>which ones, and how?</code>
     * вместо <em>). looksLikeProse() отличает такой случай и оставляет текст
     * видимым переводчику вместо маскировки всего блока.
     *
     * Плейсхолдер вплотную к слову без пробела (напр. "knows ${0}current${1}
     * Laravel") заставляет Google Translate трактовать их как один
     * непереводимый кусок и пропускать слово между ними — воспроизведено на
     * реальном тексте с <em>current</em>. Поэтому после маскировки там, где
     * пробела перед/после токена ещё нет, он добавляется — ровно один, не
     * задваивая уже существующий (иначе после восстановления в хранимом
     * HTML накапливались бы лишние пробелы).
     *
    ...
    private function maskMarkup(string $html): array
    {
        ...
        $html = preg_replace_callback(
            '~<code\b[^>]*>(.*?)</code>~is',
            fn (array $m) => $this->looksLikeProse($m[1]) ? $m[0] : $mask($m[0]),
            $html
        );
        ...
        $html = preg_replace('/(\S)(\$\{\d+\})/', '$1 $2', $html);
        $html = preg_replace('/(\$\{\d+\})(\S)/', '$1 $2', $html);
