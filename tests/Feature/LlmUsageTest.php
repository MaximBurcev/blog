...
class LlmUsageTest extends TestCase
{
    ...
    public function test_quota_pause_survives_a_deploy(): void
    {
        ...
        try {
            ...
            // В тестах у этого хранилища драйвер array (phpunit.xml), так что
            // состояние и так не переживёт тест. Чистим на случай, если драйвер
            // когда-нибудь вернут к файловому: тогда предохранитель протекал бы
            // в следующие тесты и падал бы не тот тест, что сломался.
            Cache::store('translation')->clear();
        }
    }
