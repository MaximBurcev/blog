...
Бесплатный тир — это **20 запросов в сутки** на `gemini-3.6-flash`, а статья
стоит минимум два (заголовок и тело, плюс по запросу на каждый кусок длиннее
20 000 символов). То есть без платного тарифа модель переводит примерно десять
статей в день, остальное штатно уходит скрейперу.
